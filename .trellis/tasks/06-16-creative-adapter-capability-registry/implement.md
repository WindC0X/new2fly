...
4. [x] new-api: add dry-run redacted request preview for mock and GrsAI fixture-backed presets. _(Mock dry-run plus GrsAI fixture-only dry-run request/response shape added; no provider endpoint/baseURL/key is persisted or returned.)_
...
6. [x] new-api validator tests: duplicate id, unknown preset/template, wrong group/channel/modality, disabled channel, forbidden schema id, hidden user-submitted field, raw option bypass. _(Covered by parser/validator, controller option-block, resolver/userParams tests, and channel lookup tests for missing/disabled/enabled locked channels.)_
...
9. [x] Provider fixtures: mark Duomi blocked; add GrsAI request/parser/redaction fixtures from local evidence only. _(Duomi live presets remain blocked; GrsAI fixture-only preset/template accepts sanitized dry-run config, previews request body, parses fixture response statuses/results without returning provider URLs, and is hidden from public catalog/submit.)_
10. [x] No-provider-call gate: dry-run/fixture code uses mock transport or panic-on-provider-call test hooks; Phase B cannot reach Duomi/GrsAI network endpoints. _(Dry-run and fixture helpers are pure local shape builders/parsers; AST regression gate covers dry-run functions and forbids HTTP/client/channel/key/baseURL/endpoint material.)_
