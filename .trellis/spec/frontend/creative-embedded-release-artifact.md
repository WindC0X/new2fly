...
- `apps/web/public/version.json` buildTime/gitCommit changes caused by local `pnpm build:web` are build side effects unless intentionally part of an Opentu source release; do not commit source timestamp churn blindly. For embedded `new-api` release candidates, the important provenance is the `gitCommit` copied into the embedded dist `version.json` after building from the pinned OpenTU source commit.
- Avoid a provenance self-reference loop: if OpenTU source commit `A` is built and `version.json.gitCommit=A`, do not create a new OpenTU commit `B` only to update that generated source-side `version.json` unless the release process intentionally treats `B` as a source release and rebuilds again. Prefer keeping OpenTU source clean at `A`, syncing `dist/apps/web` into `new-api`, and pinning `A` in the runbook while `new-api` embeds dist provenance `A`.
...
- `new-api` Go embed directives for Creative dist must use `all:web/creative/dist` (and test fixtures `all:...`) when the artifact may contain `_`-prefixed Vite files such as `__vite-browser-external-*.js`; plain directory `go:embed web/creative/dist` skips `_`/`.` entries and can pass local file checks while the production binary still returns 404.
...
- Embedded dist `version.json.gitCommit` differs from the OpenTU source commit pinned in the runbook -> fail release readiness; rebuild/sync from the pinned source commit and update the `new-api` dist commit, not just the runbook.
- `pnpm build:web` dirties tracked OpenTU `apps/web/public/version.json` after the source candidate commit and committing it would move the pinned source ref without rebuilding -> do not blindly commit it; either treat it as an intentional source release and rebuild/resync, or restore the generated source-side file and keep the embedded dist provenance pointing at the source commit used for the build.
...
- A `_`-prefixed generated asset exists in both `new-api` dist directories but returns 404 from the built binary/container -> fail; check the Go embed directive and use `all:` so the binary includes underscore-prefixed files.
